...
class Bank:
    def __init__(self,name,unit,value=0):
        self.name = name
        self.unit = unit
        self.current_account = unit(value)
        #instance of a Currency class
    ...
    def deposit(self, currency_instance):
        #if currency same
        if isinstance(currency_instance, self.unit):
            self.current_account.value += currency_instance.value
            return "successful deposit"
        # #if currency not same
        else:
...
if __name__ == "_main__":
    usbank = Bank("U.S. Bank", Dollar, 300)
    europe_bank = Bank("European Central Bank", Pound, 75)
    bank_of_china= Bank("Bank of China", Yuan, 1200)
